...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'DuongRuYi Python-Flask Web App'
    ...
    if _user_id:
        ...
        if form.validate_on_submit():
            ...
            if status.description == "Đang thực hiện":
                project.status_id = 2  
        # Giả sử 2 là mã trạng thái 'Đang thực hiện' trong bảng trạng thái của project
...
def searchProject():
    ...
    if _user_id:
        user = db.session.query(models.User).filter_by(user_id=_user_id).first()
        search_query = request.form['search_query']  # tìm kiếm từ form
...
def search_by_name_task():
    ...
    if _user_id:
        user = db.session.query(models.User).filter_by(user_id=_user_id).first()
        search_name_query = request.form['search_name_query']  # tìm kiếm từ form
...
def search_by_name_project():
    ...
    if _user_id:
        ...
        # Tìm kiếm các task có mô tả chua tìm kiếm
